...
async function testHMAC(): Promise<void> {
    ...
    assert(
        hmacWithBytes.unwrap() === hmacSha256.unwrap(),
        'HMAC with string and bytes should match',
    );
